...
contract sedLeo is ERC20, Ownable {
    ...
    uint256 private swapTokensAtAmount = 2400 * (10**18);
    ...
    uint256 public buyLiquidityFee = 600;
    uint256 public buyVault1Fee = 200;
    uint256 public buyVault2Fee = 200;
    uint256 public buyTotalFees = buyLiquidityFee + buyVault1Fee + buyVault2Fee;

    uint256 public sellFeesCollected = 0;
    uint256 public sellLiquidityFee = 200;
    uint256 public sellVault1Fee = 600;
    uint256 public sellVault2Fee = 600;
    uint256 public sellTotalFees = sellLiquidityFee + sellVault1Fee + sellVault2Fee;
    ...
    event vault1Updated(address indexed newVault1, address indexed oldVault1);
    ...
    modifier onlyFarmOrOwner{
        require(msg.sender == owner() || msg.sender == farm);
        _;
    }
    ...
    function updateVault1(address newVault1) external onlyOwner {
        require(newVault1 != vault1, "The vault1 wallet is already this address");
        emit vault1Updated(newVault1, vault1);
        vault1 = newVault1;
    }

    function updateVault2(address newVault2) external onlyOwner {
        require(newVault2 != vault2, "The vault2 wallet is already this address");
        emit vault1Updated(newVault2, vault2);
        vault2 = newVault2;
    }
    ...
    function swapFeesIfAmountIsReached(address from, address to) private {
        ...
        if (
            ...
            uint256 vault1Tokens = (buyFeesCollected * buyVault1Fee) / buyTotalFees;
            vault1Tokens += (sellFeesCollected * sellVault1Fee) / sellTotalFees;
            if (vault1Tokens > 0) swapAndSendToFeeVault1(vault1Tokens);

            uint256 vault2Tokens = (buyFeesCollected * buyVault2Fee) / buyTotalFees;
            vault2Tokens += (sellFeesCollected * sellVault2Fee) / sellTotalFees;
            if (vault2Tokens > 0) swapAndSendToFeeVault2(vault2Tokens);
    ...
    function swapAndSendToFeeVault1(uint256 tokens) private {
    ...
    function mint(address to, uint256 amount) external onlyFarmOrOwner{
        _mint(to, amount);
        _startSupply += amount;
    }

    function updateFarm(address _farm) external onlyOwner{
        farm = _farm;
    }
}